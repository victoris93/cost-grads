...
task_data = task_data.rename(columns={'Subject': 'participant_id'})
task_data.to_csv('./task_data/all_data_raw.csv', index=False)

data = prepare_distance_task_data('./task_data/all_data_raw.csv', 'distances.csv', False)
dem_vars = pd.read_csv('./SIFI_full.csv')[["participant_id", "sex", "age"]]
data = data.merge(dem_vars, on='participant_id')
